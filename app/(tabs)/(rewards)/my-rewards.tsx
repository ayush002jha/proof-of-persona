...
export default function MyRewardsScreen() {
  ...
  const fetchMyRewards = useCallback(async () => {
    ...
    try {
      const queryMsg = {
        UserDocuments: { owner: account.bech32Address, collection: "rewards" },
      };
    ...
    Alert.alert("Confirm Deletion", "This action is permanent.", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            const deleteMsg = {
              Delete: { collection: "rewards", document: rewardId },
            };
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={isLoading} onRefresh={fetchMyRewards} />
        }
      >
