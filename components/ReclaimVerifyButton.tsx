...
interface ReclaimVerifyButtonProps { provider: PersonaProvider; onVerificationComplete: () => void; }

export const ReclaimVerifyButton: React.FC<ReclaimVerifyButtonProps> = ({ provider, onVerificationComplete }) => {
  ...
  const handleVerification = async () => {
    if (!account?.bech32Address || !signingClient || !queryClient) {
      return Alert.alert("Error", "Wallet not connected or client not ready. Please try again.");
    }
      ...
      if (error instanceof ReclaimVerification.ReclaimVerificationException) {
        switch (error.type) {
            case ReclaimVerification.ExceptionType.Cancelled:
                ...
                break;
            default:
                Alert.alert("Verification Failed", error.message || "An unknown Reclaim error occurred.");
        }
      } else {
  ...
  return (
    <View className="my-2">
      <Button title={getButtonTitle()} onPress={handleVerification} disabled={status !== 'idle'} />
